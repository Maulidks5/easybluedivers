...
Route::middleware('auth')->prefix('admin')->group(function () { Route::get('/', DashboardController::class); Route::post('/logout', [AuthController::class, 'destroy']); });
...
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('experiences', DiveExperienceController::class)->only(['index', 'store', 'update', 'destroy']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('courses', CourseController::class)->only(['index', 'store', 'update', 'destroy']); Route::resource('prices', PricePackageController::class)->only(['index', 'store', 'update', 'destroy']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('reviews', ReviewController::class)->only(['index', 'store', 'update', 'destroy']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('gallery', GalleryImageController::class)->only(['index', 'store', 'update', 'destroy']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('dive-planner', DiveScheduleDayController::class)->only(['index', 'store', 'update', 'destroy']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('experience-categories', ExperienceCategoryController::class)->only(['index', 'store', 'update', 'destroy']); Route::resource('experience-slots', ExperienceSlotController::class)->only(['index', 'store', 'update', 'destroy']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::get('settings', [SiteSettingController::class, 'edit']); Route::put('settings', [SiteSettingController::class, 'update']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::get('content', [SiteContentController::class, 'index']); Route::put('content', [SiteContentController::class, 'update']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::get('home-hero', [HomeHeroMediaController::class, 'edit']); Route::put('home-hero', [HomeHeroMediaController::class, 'update']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('faqs', FaqController::class)->only(['index','store','update','destroy']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('team', TeamMemberController::class)->only(['index','store','update','destroy']); Route::resource('dive-sites', DiveSiteController::class)->only(['index','store','update','destroy']); Route::resource('partners', PartnerLogoController::class)->only(['index','store','update','destroy']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('users', UserManagementController::class)->only(['index','store','update','destroy']); });
Route::middleware('auth')->prefix('admin')->group(function () { Route::resource('enquiries', AdminContactEnquiryController::class)->only(['index', 'update', 'destroy']); });
